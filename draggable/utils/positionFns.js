...
var positionFns = (function () {
  
  const { getTouch, innerWidth, innerHeight, offsetXYFromParent, outerWidth, outerHeight } = domFns;
  const { isNum, int } = shims;

  return {
    getBoundPosition: getBoundPosition,
    snapToGrid: snapToGrid,
    canDragX: canDragX,
    canDragY: canDragY,
    getControlPosition: getControlPosition,
    createCoreData: createCoreData,
    createDraggableData: createDraggableData
  }
  ...
  function getControlPosition (e, touchIdentifier, draggableCore) {
    // console.log('getControlPosition....', e, touchIdentifier, draggableCore)
    const touchObj = typeof touchIdentifier === 'number' ? getTouch(e, touchIdentifier) : null;
    if (typeof touchIdentifier === 'number' && !touchObj) return null; // not the right touch
    // -- TODO REMOVE LINE
    // const node = findDOMNode(draggableCore);
    
    const node = draggableCore.dom

    // User can provide an offsetParent if desired.
    const offsetParent = draggableCore.attrs.offsetParent || node.offsetParent || node.ownerDocument.body;
  ...
  function createCoreData (draggable, x, y) {
    // console.warn('creatCoreData....', draggable, x, y)
  ...
  function findDOMNode (draggable) {
    // console.warn('findDOMNode: ', draggable)
    const node = draggable.dom
